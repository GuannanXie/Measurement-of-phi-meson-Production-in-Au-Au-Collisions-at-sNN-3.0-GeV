...
\title{Measurement of $\phi(1020)$ and $\Xi^{-}$ Production in Au+Au Collisions at ${\sqrt{s_{\rm NN}} = \rm{3\,GeV}}$}% Force line breaks with \\
...
A study based on THERMUS~\cite{THERMUS_WHEATON200984} calculations using strangeness CE with $r_c$ = 3.2\,fm ($T_{ch}$ = 80\,MeV, $\mu_B$ = 720\,MeV, $\mu_S$ = 240\,MeV, $\mu_Q$ = 0, $\gamma_s = 1$ and $r$ = 6.2\,fm from the previous parameterization~\cite{CE_Cleymans}) predicates the $\phi/\Xi^-$ ratio $\sim$2.15, which is a bit larger than the measurement. The measured $\phi/\Xi^-$ ratio at 3\,GeV favors a $r_c$ parameter between 3.2 and 4.2\,fm. Note that the detailed CE calculation, parameterization and implementation in our prediction may be different as the one used in Fig.~\ref{fig:phi2Kratio} (a).  
